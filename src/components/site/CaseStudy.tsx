...
/** Project page: the name and the picture share one row. */
export function CaseStudy({ content, previous, next, previewBanner }: Props) {
  const timeline = content.dateRange || content.year;
  const summary = [content.subtitle || content.categories[0], timeline, content.projectStatus].filter((value) => value?.trim()).join("  ·  ");
  ...
  return (
    <article>
      {previewBanner}
      <Container>
        <header className="grid items-start gap-8 pb-10 pt-20 md:grid-cols-12 md:gap-10 md:pt-24">
          <div className={hasCover ? "md:col-span-5" : "md:col-span-8"}>
            ...
            {summary ? <p className="mt-3 max-w-[36ch] text-sm leading-relaxed text-fg-muted">{summary}</p> : null}
            {name ? <p className="mt-5 text-base font-medium text-fg">{name}</p> : null}
            {links.length ? (
          ...
          {content.coverVideo ? (
            <div className="relative aspect-[4/3] overflow-hidden bg-bg-elevated md:col-span-7">
              <MediaVideo media={content.coverVideo} poster={content.cover} className="h-full w-full object-cover" />
            </div>
          ) : content.cover ? (
            <div className="relative aspect-[4/3] overflow-hidden bg-bg-elevated md:col-span-7">
              <MediaImage media={content.cover} fill priority sizes="(min-width: 768px) 58vw, 100vw" quality={90} />
            </div>
          ) : null}
        </header>
      </Container>
