...
export async function updateMediaDetails(id: string, input: { title: string; altText: string }) {
  return adminAction(async () => {
    ...
    const { error } = await supabase
      .from("media")
      .update({ title: input.title.slice(0, 200), alt_text: input.altText.slice(0, 500) })
      .eq("id", id);
    if (error) throw new Error(error.message);
  });
}
...
export async function getMediaUsage(id: string) {
  return adminAction(async () => findMediaUsage(id));
}

async function findMediaUsage(id: string): Promise<MediaUsage[]> {
  const supabase = await createClient();
  ...
  const [projects, drafts, sets, profile, settings] = await Promise.all([
    ...
    supabase.from("project_drafts").select("project_id, content, projects(content)"),
    ...
    supabase.from("site_settings").select("og_image, favicon").eq("id", 1).maybeSingle(),
  ]);

  for (const p of projects.data ?? []) {
    if (mentions(p.content)) {
      const title = (p.content as { title?: string })?.title || p.slug;
      usage.push({ kind: "project", label: `${title} (live)`, href: `/admin/projects/${p.id}` });
    }
  }
  for (const d of drafts.data ?? []) {
    if (mentions(d.content)) {
      const title = (d.content as { title?: string })?.title || "Untitled project";
      usage.push({ kind: "draft", label: `${title} (draft)`, href: `/admin/projects/${d.project_id}` });
    }
  }
  for (const s of sets.data ?? []) {
    if (mentions(s.before_image) || mentions(s.after_image)) {
      usage.push({ kind: "photography", label: s.title || "Untitled photo set", href: `/admin/photography/${s.id}` });
    }
  }
...
/**
 * Deletes a file from storage and the library, and removes it from every
 * project, draft, photo set, the profile and the settings that used it.
 */
export async function deleteMedia(id: string) {
  return adminAction(async () => {
    ...
    for (const p of projects.data ?? []) {
      if (mentions(p.content)) {
        await supabase.from("projects").update({ content: scrub(p.content, id) as Json }).eq("id", p.id);
      }
    }
    for (const d of drafts.data ?? []) {
      if (mentions(d.content)) {
        await supabase.from("project_drafts").update({ content: scrub(d.content, id) as Json }).eq("project_id", d.project_id);
      }
    }
    for (const s of sets.data ?? []) {
      if (mentions(s.before_image) || mentions(s.after_image)) {
    ...
    const storage = await supabase.storage.from(MEDIA_BUCKET).remove([media.path]);
